...
pub struct App {
    ...
    pub updater: Box<dyn Fn(&mut App) + Send + Sync>,
...
pub struct SubApp {
    ...
    pub updater: Box<dyn Fn(&mut World, &mut App) + Send + Sync>,
}
...
impl App {
    ...
    pub fn set_updater(&mut self, updater: impl Fn(&mut App) + Send + Sync + 'static) -> &mut Self {
    ...
    pub fn add_sub_app(
        ...
        label: impl AppLabel,
        app: App,
        sub_app_runner: impl Fn(&mut World, &mut App) + Send + Sync + 'static,
    ) -> &mut Self {
